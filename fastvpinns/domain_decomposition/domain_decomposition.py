"""
This module implements domain decomposition for solving partial differential equations using Variational Physics-Informed Neural Networks (VarPINNs).

Authors: Thivin Anandh, Divij Ghose
Revision History:
- First implementation: 16/01/2024
"""

import tensorflow as tf
from fastvpinns.FE.fespace2d import *
from fastvpinns.Geometry import geometry_2d
import numpy as np
...
class DomainDecomposition():
    ...
    def __init__(self, domain):
        ...
        self.domain = domain
        self.cells_in_domain = self.domain.cells_in_domain        

        self.cells_in_domain_2D = self.cells_in_domain.reshape((self.domain.n_cells_x, self.domain.n_cells_y))
        self.cells_in_domain_2D = np.flip(self.cells_in_domain_2D, axis=0)

        self.blocks_in_domain = self.domain.domains

        self.blocks_shared_by_cells = self.id_blocks_shared_by_cells() # This is a dictionary with cell_id as key and list of blocks as value. A block is a collection of cells.

        self.halo_cells_in_blocks = self.id_halo_cells() # This is a dictionary with block_id as key and list of halo cells as value. A halo cell is a cell that is shared by more than one block.
    ...
    def print_details(self):
        ...
        values = [self.domain.n_cells_x, self.domain.n_cells_y, self.domain.kernel_size_row, self.domain.kernel_size_col, self.domain.stride_row, self.domain.stride_col]
        ...
        rows = np.arange(0, len(self.blocks_in_domain))
        values = self.blocks_in_domain
    ...
    def id_blocks_shared_by_cells(self):
        ...
        for cell in self.cells_in_domain:
            for block_id, block in enumerate(self.blocks_in_domain):
                if cell in block:
    ...
    def plot_domains(self):
        ...
        for block_id, block in enumerate(self.blocks_in_entire_domain):
            ...
            plt.text(centroid[0]-0.1,centroid[1],f"Block_id: {block_id}",fontsize=20, bbox=dict(facecolor='white', alpha=0.9))
            ...
            output_file_name = Path(self.output_path + "/iter_plots/" + "block_" + str(block_id) + ".png")
