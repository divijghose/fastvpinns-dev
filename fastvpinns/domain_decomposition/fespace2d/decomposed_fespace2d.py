...
class DecomposedFespace2D(Fespace2D):
    """
    This class will hold the information of the decomposed FE spaces
    of all the cells within the given mesh
    """
    def __init__(self, cells_points,  cell_type: str, fe_order: int, fe_type: str, quad_order: int, quad_type: str, fe_transformation_type: str,  \
                    forcing_function, output_path: str, generate_mesh_plot = False, x_minmax=None, y_minmax=None):
        """
        The constructor of the fespace2d class.
        """
        self.mesh = []
        self.boundary_points = []
        self.bound_function_dict = {}
        self.bound_condition_dict = {}
        self.cells_points = cells_points
        self.cell_type = cell_type
        self.fe_order = fe_order
        self.fe_type = fe_type
        self.quad_order = quad_order
        ...
        self.x_mean = (self.x_max + self.x_min)/2.0
        self.x_span = (self.x_max - self.x_min)/2.0
        self.y_min = y_minmax[0]
        self.y_max = y_minmax[1]
        self.y_mean = (self.y_min + self.y_max)/2.0
        self.y_span = (self.y_max - self.y_min)/2.0


        super().__init__(self.mesh, self.cells_points, self.boundary_points, self.cell_type, self.fe_order, self.fe_type, self.quad_order, self.quad_type, self.fe_transformation_type, self.bound_function_dict, self.bound_condition_dict, self.forcing_function, self.output_path, self.generate_mesh_plot)
